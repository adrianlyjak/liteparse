...
mod open_document {
    ...
    async fn parse_pages_validates_the_entire_selection_before_parsing() {
        ...
        for (pages, expected) in [
            ...
            let error = match document.parse_pages(pages).await {
                Ok(_) => panic!("an invalid selection should fail before parsing"),
                Err(error) => error,
            };
    ...
    #[tokio::test]
    #[serial]
    async fn parse_pages_applies_max_pages_after_normalization() {
    ...
    #[tokio::test]
    #[serial]
    async fn parse_pages_ignores_configured_target_pages() {
        let document = LiteParse::new(LiteParseConfig {
            ocr_enabled: false,
            quiet: true,
