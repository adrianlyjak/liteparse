...
import { loadFirstAvailable, nativeBindingCandidates } from "./native-loader.js";
...
function loadNative(): NativeBindings {
  ...
  const binding = loadFirstAvailable<NativeBindings>(
    nativeBindingCandidates(platform, arch, searchDirs),
    require,
  );
  if (binding) {
    return binding;
