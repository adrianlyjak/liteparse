...
office and image inputs are converted to a temporary PDF once. The temporary
file remains alive until `close()` finishes.
...
Open a document once when you need to parse several page selections. Supported
office and image inputs are converted to a temporary PDF once. The context
manager closes the document and removes the temporary file when the block exits.
...
Open a document once when you need to parse several page selections. Native
builds convert supported office and image inputs to a temporary PDF once. The
temporary file remains alive until `close()` finishes. WASM accepts PDF bytes
only.
