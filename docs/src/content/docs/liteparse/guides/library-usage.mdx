...
Open a PDF once when you need to parse several page selections. Open documents
accept PDF paths or buffers and do not convert other file formats.
...
Open a PDF once when you need to parse several page selections. Open documents
accept PDF paths or bytes and do not convert other file formats. The context
manager closes the document when the block exits.
...
Open a PDF once when you need to parse several page selections. `open_document`
accepts PDF paths or bytes and does not convert other file formats.
...
let parser = LiteParse::new(LiteParseConfig::default());
let document = parser.open_document(PdfInput::Path("document.pdf".into()))?;
